...
import { concatClips, probeHasAudio, renderNormalizedClip, type Canvas } from './ffmpeg'
import { log } from './log'
...
export function makeRenderHandler(deps: {
  ...
  return async function render({ job, setProgress }: JobContext): Promise<void> {
    ...
    const maxDurationMs = variant.type === 'teaser' ? 30_000 : undefined
    ...
    try {
      ...
      // Concat (lossless) and encrypt the deliverable into the vault.
      ...
      setProgress(1)
      log.info('render.ready', { variantId, clips: clipPaths.length, durationMs: totalMs })
    } catch (e) {
